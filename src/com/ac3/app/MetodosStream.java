...
public class MetodosStream {
	...
	public static void main(String[] args) {
		...
		// de unir
		// los streams generados por la aplicación de una función de cada elemento.
		s("flatMap()");
		...
		// peek(Consumer<? super T> c) -> Devuelve un stream idéntico y realiza alunga
		// operación con los objetos intermedios
		s("peek()");
		st3.peek(d -> System.out.print(d + "- peek: ")).forEach((s) -> System.out.print(s + " "));

		// sorted() // sorted(Comparator<? super Integer> c) -> Devuelve el stream
		...
		System.out.println(); // Para separar los streams
		st2 = Arrays.stream(cads); // Reinicio el Stream
		// Podemos usar los métodos estáticos de Comparator para establecer otros
		// criterios
		...
		// OptionalInt min() -> trabaja en streams primitivos y devuelve el valor menor
		s("min()");
		st = nums.stream(); // Reinicio el Stream()
		System.out.println(st.mapToInt(a -> a).min().getAsInt());
